...
import { downloadCsv, fetchOutOfStock } from '../api';
...
export function StockOutPage() {
  ...
  const { data = [] } = useQuery({
    queryKey: ['stock', 'out'],
    queryFn: () => fetchOutOfStock() as Promise<Row[]>,
  });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">{t('product.outOfStock')}</h1>
        <button
          onClick={() => void downloadCsv('/stock/out-of-stock', 'gutaran.csv')}
